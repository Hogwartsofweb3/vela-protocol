...
declare_id!("MoCKondo11111111111111111111111111111111111");
...
pub mod mock_ondo {
    ...
    pub fn deposit_and_mint(ctx: Context<DepositAndMint>, amount: u64) -> Result<()> {
        ...
        let mint_cpi = CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            MintTo {
                mint: ctx.accounts.musdy_mint.to_account_info(),
                to: ctx.accounts.user_musdy_account.to_account_info(),
    ...
    pub fn burn_and_withdraw(ctx: Context<BurnAndWithdraw>, amount: u64) -> Result<()> {
        // Burn musdy from user
        let burn_cpi = CpiContext::new(
            ctx.accounts.token_program.to_account_info(),
            Burn {
                mint: ctx.accounts.musdy_mint.to_account_info(),
                from: ctx.accounts.user_musdy_account.to_account_info(),
                authority: ctx.accounts.user.to_account_info(),
            },
        );
...
pub struct DepositAndMint<'info> {
    ...
    #[account(mut)]
    pub musdy_mint: Account<'info, Mint>,
    #[account(mut)]
    pub user_usdc_account: Account<'info, TokenAccount>,
    #[account(mut)]
    pub vault_usdc_account: Account<'info, TokenAccount>,
    #[account(mut)]
    pub user_musdy_account: Account<'info, TokenAccount>,
